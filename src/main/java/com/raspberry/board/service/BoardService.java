...
public class BoardService {
    ...
    public ModelAndView getBoardList(SearchDto sdto, HttpSession session){
        ...
        mv.addObject("paging", pageHtml); //"paging" 은 boardList에 있는 내용

        //세션에 필요 정보 저장(pageNum, 검색관련 정보)
        //페이지 번호 지정 - 글쓰기 또는 상세보기 화면에서 목록으로 돌아갈 때
    ...
    private String getPaging(SearchDto sdto) {
        ...
        PagingUtil paging = new PagingUtil(maxNum, sdto.getPageNum(), sdto.getListCnt(),
                                            pageCnt, listName); //PagingUtil에 있는 순서대로

        pageHtml = paging.makePaging();
        return pageHtml;
    }

    public String boardWrite(List<MultipartFile> files, BoardDto board,
        ...
        try {
            ...
            if (point > 100) {//100초과하지 않도록
                point = 100;
            }
            member.setM_point(point);//변경한 포인트를 다시 넣음

            //DB member 테이블 수정(update)
            mDao.updateMemberPoint(member);
            session.setAttribute("mb", member);
            //같은 이름으로 세션에 저장하면 기존것에 덮어쓰기가 된다.
    ...
    public String boardDelete(Integer b_num, HttpSession session, RedirectAttributes rttr){
        ...
        try {
            //삭제순서는 댓글/파일정보 다음에 게시글을 삭제
            //따라서 댓글이나 파을을 먼저 삭제한다.
    ...
    private void deleteFile(Integer b_num, HttpSession session) throws Exception {
        ...
        //DB에서 같은 게시물에 해당하는 sysname 목록을 받아서 반복적으로 삭제 처리
    ...
    //게시글 수정
    public String updateBoard(List<MultipartFile> files, BoardDto board,
                                HttpSession session, RedirectAttributes rttr){

...
public class BoardController {
    ...
    //core.io <Resource>사용
    @GetMapping("download")
    public ResponseEntity<Resource> fileDownload(BfileDto bfile,
    ...
    public ReplyDto replyInsert(ReplyDto reply){
        ...
        reply = bServ.replyInsert(reply);//서비스 처리 부분
        return reply;
    }
    ...
    public ModelAndView updateForm(Integer b_num){
        log.info("updateForm()");
        mv = bServ.updateForm(b_num); //서비스에서 처리
        return mv;
    }

    @PostMapping("updateProc")
    public String updateProc(@RequestPart List<MultipartFile> files, BoardDto board,

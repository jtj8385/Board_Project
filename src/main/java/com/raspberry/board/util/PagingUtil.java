...
public class PagingUtil {
    ...
    private String listName;//목록 페이지가 여러개인 경우 페이징 처리할 목록 페이지 url

    //페이징용 html코드를 만드는 메소드
    public String makePaging(){
        String page = null;
        StringBuffer sb = new StringBuffer();

        //1. 전체 페이지 개수 구하기(페이지 당 listCnt 만틈 출력)
        //listCnt가 5일 때
        //전체 데이터가 4개 : 1페이지
        //전체 데이터가 6개 : 2페이지
        ...
        //ex) 두번째 그룹의 시작번호 = 2 * 5 - (5 - 1) = 6

        //4. 현재 보이는 페이지 그룹의 마지막 번호 구하기
        int end = (curGroup * pageCnt) >= totalPage ?
                totalPage : curGroup * pageCnt;
        //totalPage 9, curGroup * pageCnt 10 → totalPage 가 마지막 번호

        //이전 버튼 처리(시작번호가 1일때는 버튼생성 X)
        if (start != 1){
